...
console.log("");
const getCommit = (eventid) => {
    for (let i = 0; i < githubData.length; i++) {
        const event = githubData[i];
        if (event.id === eventid) {
            console.log(event);
            let allCommits = event.payload.commits;
            console.log(allCommits);
            for (let j = 0; j < allCommits.length; j++) {
                const element = allCommits[j];
                console.log(allCommits[j].sha, allCommits[j].message);
            }
        }
    }
}
getCommit("8030403992");

getCommit("8032498553");